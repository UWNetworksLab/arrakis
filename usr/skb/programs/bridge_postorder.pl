...
% the PCI tree is constructed by adding just the next child device to the
% current bridge. they are not sorted in any particular way. the tree gets
% traversed in a postorder manner and resources are allocated immediately.

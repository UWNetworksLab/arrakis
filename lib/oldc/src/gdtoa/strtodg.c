...
rvOK
 ...
 double d; FPI *fpi; Long *exp_; ULong *bits; int exact, rd, *irv;
#else
 (double d, FPI *fpi, Long *exp_, ULong *bits, int exact, int rd, int *irv)
#endif
{
	Bigint *b;
...
strtodg
#ifdef KR_headers
	(s00, se, fpi, exp_, bits)
	CONST char *s00; char **se; FPI *fpi; Long *exp_; ULong *bits;
#else
	(CONST char *s00, char **se, FPI *fpi, Long *exp_, ULong *bits)
#endif
{
...
#define dplen 1
 ...
 break2:
	if (*s == '0') {
		...
		switch(s[1]) {
		  case 'x':
		  case 'X':
			irv = gethex(&s, fpi, exp_, &rvb, sign);
 ...
 dig_done:
	...
	if (nbits <= P && nd <= DBL_DIG) {
		if (!e) {
			if (rvOK(dval(rv), fpi, exp_, bits, 1, rd, &irv))
				goto ret;
			}
		else if (e > 0) {
			if (e <= Ten_pmax) {
				...
				if (rvOK(dval(rv), fpi, exp_, bits, i, rd, &irv))
					goto ret;
				e1 -= e;
				goto rv_notOK;
#endif
...
#endif
				if (rvOK(dval(rv), fpi, exp_, bits, 0, rd, &irv))
					goto ret;
				e1 -= e2;
				}
			}

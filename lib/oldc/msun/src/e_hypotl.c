...
#include <sys/cdefs.h>
__FBSDID("$FreeBSD: src/lib/msun/src/e_hypotl.c,v 1.2 2008/03/31 13:11:45 das Exp $");
...
{
	long double a=x,b=y,t1,t2,y1_var,y2,w;
	int32_t j,k,ha,hb;
	...
	if (w>b) {
	    ...
	    y1_var = b;
	    union IEEEl2bits uv;
	    uv.e = y1_var; uv.bits.manl = 0; y1_var = uv.e;
	    y2 = b - y1_var;
	    t1 = a;
	    uv.e = t1; uv.bits.manl = 0; t1 = uv.e;
	    t2 = a - t1;
	    w  = sqrtl(t1*y1_var-(w*(-w)-(t1*y2+t2*b)));
	}
	if(k!=0) {
	    u_int32_t high;
	    t1 = 1.0;

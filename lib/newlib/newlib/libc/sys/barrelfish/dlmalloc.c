...
#define MORECORE_CANNOT_TRIM  // undef for lesscore
#define HAVE_MMAP 0
...
# define time(x) rdtsc()
#else
# define time(x) (4) // chosen by fair dice roll.
...
static void *my_morecore(intptr_t nb)
{
...
#define INITIAL_LOCK(sl)  bf_init_lock(sl)
#define ACQUIRE_LOCK(sl)  bf_acquire_lock(sl)
#define RELEASE_LOCK(sl)  bf_release_lock(sl)
#define TRY_LOCK(sl) bf_try_lock(sl)
static MLOCK_T malloc_global_mutex = THREAD_MUTEX_INITIALIZER;

static FORCEINLINE int bf_init_lock (MLOCK_T *sl) {
    thread_mutex_init(sl);
    return 0;
}

static FORCEINLINE int bf_acquire_lock (MLOCK_T *sl) {
...
static FORCEINLINE int bf_try_lock (MLOCK_T *sl) {
    return thread_mutex_trylock(sl) ? 1 : 0;
}
...
#define CORRUPTION_ERROR_ACTION(m)  printf("%s %s %d: corruption error action\n", __FILE__, __func__, __LINE__)
//; reset_on_error(m)
#define USAGE_ERROR_ACTION(m, p) printf("%s %s %d: usage error action p: %p\n", __FILE__, __func__, __LINE__, p)
      ...
      if (!is_extern_segment(sp)) {
        if (is_mmapped_segment(sp)) {
          if (HAVE_MMAP &&
              ...
              //IK            size_t newsize = sp->size - extra;
